...
function syncNavHeight() {
  ...
  if (!header) return;
  const apply = () => {
    document.body.style.setProperty("--top-nav-height", `${header.offsetHeight}px`);
  };
  apply();
    ...
    days.forEach((d) => {
      ...
      items.forEach((t) => {
        ...
        if (cat) {
          ...
          badge.textContent = labelForCategory(cat);
          chip.append(badge);
        }
        ...
        if (Math.abs(delta) < 60 * 60 * 1000) {
          ...
          near.className = delta >= 0 ? "badge badge-soon" : "badge badge-late";
          near.textContent = delta >= 0 ? "קרוב" : "עבר לפני רגע";
          chip.append(near);
        }
